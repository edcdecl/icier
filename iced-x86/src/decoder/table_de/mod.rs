...
struct TableDeserializer<'a> {
	reader: DataReader<'a>,
	handler_reader: fn(&mut TableDeserializer<'_>, result: &mut Vec<(OpCodeHandlerDecodeFn, &'static OpCodeHandler)>),
...
impl<'a> TableDeserializer<'a> {
	...
	fn deserialize(&mut self) {
		while self.reader.can_read() {
			...
			let kind = unsafe { mem::transmute::<_, SerializedDataKind>(self.reader.read_u8() as u8) };
			match kind {
...
#[must_use]
pub(super) const fn read_legacy() -> Vec<(OpCodeHandlerDecodeFn, &'static OpCodeHandler)> {
	let handler_reader = legacy_reader::read_handlers;
